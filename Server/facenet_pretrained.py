...
modelFile = os.path.join(package_directory, "Models/res10_300x300_ssd_iter_140000.caffemodel")
configFile = os.path.join(package_directory, "Models/deploy.prototxt.txt")
net = cv2.dnn.readNetFromCaffe(configFile, modelFile)

def get_model():
	path = os.path.join(package_directory,"Models/facenet_weights.h5")
...
def prepare(filepath):
    img_array = prepface(cv2.imread(filepath), False)
    return img_array
...
def findCosineDistance(source_representation, test_representation):
    ...
    return distance.cosine(source_representation, test_representation)
...
def prepface(face, is_face = True):
	if not is_face:
		_, face = futils.detect_face(face, net, 0.5, [], (160, 160), 0, 0, 'constant')
		if face is None: 
			return None
	else:
		face = cv2.resize(face, (160,160), cv2.INTER_AREA)
